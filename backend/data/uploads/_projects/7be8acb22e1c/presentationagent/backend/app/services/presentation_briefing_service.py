...
async def collect_supplemental_research_materials(
    ...
    for query in queries:
        searched = await web_search_execute(
            {
                "query": query,
        ...
        for rank, result in enumerate(results, start=1):
            ...
            material_id = f"research-{len(materials) + 1}"
                ...
                materials.append(
                    {
                        ...
                        "metadata": {
                            "search_query": query,
                            "search_rank": rank,
                            "search_source": searched.get("source") or "unknown",
                        },
                    }
                )
                seen_urls.add(url)
                if len(materials) >= MAX_SUPPLEMENTAL_RESEARCH_MATERIALS:
                    return materials
                continue
