"""Native PPTX renderer client and HTML-to-DeckSpec semantic adapter."""
...
NATIVE_RENDERER_URL = os.getenv("NATIVE_RENDERER_URL", "http://127.0.0.1:4100")
NATIVE_RENDERER_TIMEOUT = float(os.getenv("NATIVE_RENDERER_TIMEOUT", "60"))
NATIVE_RENDERER_HEALTH_TIMEOUT = min(NATIVE_RENDERER_TIMEOUT, 3.0)
...
def build_deckspec_from_slides(
    ...
    deck_payload = {
        ...
        "metadata": {
            ...
            "tags": [theme_id, "native-renderer", "pptx", "semantic-layout"],
...
async def render_deck_to_pptx(
    ...
    *,
    ...
    payload = {
        "deckSpec": deck_spec.model_dump(mode="json"),
        "options": {
            "author": author,
            "company": company,
            "subject": subject or deck_spec.subtitle or deck_spec.title,
            "lang": "zh-CN",
        },
    }

    try:
        async with httpx.AsyncClient(timeout=NATIVE_RENDERER_TIMEOUT) as client:
            response = await client.post(f"{NATIVE_RENDERER_URL}/render/pptx", json=payload)
            response.raise_for_status()
    except httpx.HTTPError as exc:
        logger.error("[NativeRenderer] renderer request failed: %s", exc)
        raise RuntimeError("Native renderer 服务不可用") from exc

    data = response.json()
    file_base64 = data.get("fileBase64")
    if not file_base64:
        raise RuntimeError("Native renderer 未返回 PPTX 内容")

    return base64.b64decode(file_base64), data.get("meta", {})


async def check_native_renderer_health() -> tuple[bool, str | None]:
    """Probe the native renderer health endpoint for UI capability hints."""
    try:
        async with httpx.AsyncClient(timeout=NATIVE_RENDERER_HEALTH_TIMEOUT) as client:
            response = await client.get(f"{NATIVE_RENDERER_URL}/health")
            response.raise_for_status()
    except httpx.HTTPError as exc:
        logger.warning("[NativeRenderer] health check failed: %s", exc)
        return False, f"Native renderer 未启动或不可达: {NATIVE_RENDERER_URL}"
