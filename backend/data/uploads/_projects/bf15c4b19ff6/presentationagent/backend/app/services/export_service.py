...
async def export_pdf(
    ...
    """
    通过 Playwright 渲染 reveal.js 页面并导出 PDF。
    使用 ?print-pdf 模式让 reveal.js 将所有幻灯片排列为打印布局。
    ...
    async with managed_page() as page:
        ...
        logger.info(f"[Export] PDF 渲染: {slide_count} 页幻灯片")

        # 逐页截图并合并为 PDF
        # 使用 Playwright 内置 PDF 功能（基于 Chromium 打印）
        await page.pdf(
            path=str(filepath),
            width="1280px",
            height="720px",
    ...
    logger.info(f"[Export] PDF 导出完成: {filepath}")
    return f"exports/{filename}"
...
def _inject_print_pdf_css(html: str) -> str:
    """
    为 reveal.js HTML 注入 print-pdf 所需的额外样式和配置。
    让 reveal.js 在打印模式下将所有幻灯片垂直排列。
    """
    # 在 Reveal.initialize 配置中添加 embedded: true
    # 并添加打印样式
    print_css = """
    <style>
      /* 打印模式 — 每页幻灯片独占一页 */
