...
class IntentDetectionMiddleware(AgentMiddleware):
    ...
    新增: on_request_start 中进行关键词预检测，提前设置 intent，
    避免需要等到 LLM end_turn 才识别意图（可能浪费 10+ 轮工具调用）。
    """

    # 预检测关键词 → 意图映射（按优先级排序）
    _KEYWORD_INTENT_MAP: list[tuple[re.Pattern, str]] = [
        (re.compile(
            r"ppt|演示文稿|幻灯片|deck|presentation|路演|课件|演讲稿|提案稿",
            re.IGNORECASE,
        ), "ppt"),
        (re.compile(
            r"研究|调研|分析报告|深入分析|综合分析|技术分享报告|行业报告",
            re.IGNORECASE,
        ), "research"),
    ]

    async def on_request_start(self, ctx: AgentContext) -> None:
        """基于用户消息关键词预检测意图，提前加载对应中间件能力。"""
        if ctx.intent is not None:
            return  # 已有意图，不覆盖
        for pattern, intent in self._KEYWORD_INTENT_MAP:
            if pattern.search(ctx.user_message):
                ctx.intent = intent
                ctx.set_meta("detected_intent", intent)
                logger.info(
                    f"[IntentDetection] 关键词预检测到意图: {intent} "
                    f"task={ctx.task_id}"
                )
                break
...
class SubagentOrchestrationMiddleware(AgentMiddleware):
    ...
    def _is_composite_task(self, msg: str) -> bool:
        ...
        # 显式连接词 + 动作对 → 一定是复合任务
        if has_connector and has_action_pair:
            return True
        # 含 PPT 关键词 + 研究/分析动作 → 也是复合任务（先研究再做 PPT）
        has_ppt = bool(_PPT_KEYWORDS_RE.search(msg))
        has_research_action = any(
            kw in msg_lower for kw in ["分析", "研究", "调研", "介绍", "总结", "归纳"]
        )
        if has_ppt and has_research_action:
            return True
        return False
